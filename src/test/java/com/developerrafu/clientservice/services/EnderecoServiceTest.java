...
class EnderecoServiceTest {
    ...
    void shouldSaveEnderecoRequest() {
        final EnderecoRequest endereco = EnderecoRequestMockBuilder.getBuilder().defaultValues().build();
        when(repository.save(any())).thenReturn(EnderecoMockBuilder.getBuilder().defaultValues().withClienteId(1L).build());
    ...
    void shouldReturnEnderecoResponse() {
        when(repository.findByClienteId(any())).thenReturn(Optional.of(EnderecoMockBuilder.getBuilder().defaultValues().withClienteId(1L).build()));
        when(client.getCetp(any())).thenReturn(ViaCepResponseMockBuilder.getBuilder().defaultValues().build());
    ...
    void shouldReturnEstados() {
        when(localidadeService.getEstados()).thenReturn(List.of(EstadoResponseMockBuilder.getBuilder().defaultValues().build()));
    ...
    void shoulReturnMunicipios() {
        when(localidadeService.getMunicipios(any())).thenReturn(List.of(MunicipioResponseMockBuilder.getBuilder().defaultValues().build()));
